...
trait Inside {
  ...
  def inside[T, U](value: T)(pf: PartialFunction[T, U])(implicit pos: source.Position): U = {
    ...
    if ((Inside.level.get: Any) == null) {
      // This was: `Inside.level.get == null` which got rejected since it compares Int with Null
      // I believe the test makes no sense, but to be sure I simulate the old behavior by upcasting to Any
      Inside.level.set(1)
    }
    else {
