...
private[scalatest] object DiagrammedAssertionsMacro {
  ...
  //  */
  // private[this] def getFirstLine(refl: Reflection)(expr: refl.Tree): Int = {
  //   import refl._
  //   import Term._

  //   expr match {
  //     case apply: Apply => getFirstLine(refl)(apply.fun) // If it is a Apply, we'll look into its apply.fun, which contains the qualifier (looking for the left-most).
  //     case typeApply: TypeApply => getFirstLine(refl)(typeApply.fun) // If it is a TypedApply, we'll look into its apply.fun, which contains the qualifier (looking for the left-most)
  //     case select: Select => getFirstLine(refl)(select.qualifier) // If it is a Select, we'll look at its qualifier (looking for the left)
  //     case Block(Nil, expr) => getFirstLine(refl)(expr)
  //     case Block(stats, _) => getFirstLine(refl)(stats.head)
  //     case other => refl.rootPosition.startLine // For others, just the position line number
  //   }
  // }
